$(document).ready(function () {
  var addZoomInAndButton = function (myClass) {
    $(myClass)
      .prepend("<div class='buttonMore'>View Project</div>")
      .hover(
        function () {
  ...
  var timelineToCenter = function () {
    ...
    $(".timelineEvent").each(function () {
      $(this).append(
        "<div class='iconContainer'> <img src='img/" +
          $(this).attr("id") +
          "-w.png' class='timelineIcon' /> </div>"
      );
    });
  };
  ...
  function randomOctoImageAndQuote() {
    if ($("#octo-quote").length) {
      ...
      $.getJSON("./js/octo-quotes.json", (r) => {
        ...
        randomQuote =
          Math.floor(Math.random() * (quoteMax - quoteMin)) + quoteMin;
        $("#octo-quote").html(quotes[randomQuote]);

        $(window).click(() => {
          const imgMin = 1,
            imgMax = 21;
          const randomImg =
            Math.floor(Math.random() * (imgMax - imgMin)) + imgMin;
